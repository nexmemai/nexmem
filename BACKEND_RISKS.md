...
- **Status:** ✅ FIXED. New `app/core/quota.py` provides `enforce_write_quota` (async, single shared `redis.asyncio` client, fail-open without `REDIS_URL`, fail-closed with 503 when configured but unreachable, structured 429 payload). The dependency is wired into `POST /agents/{user_id}/episodes`, `POST /agents/{user_id}/semantics`, `POST /memory/episode/write`, and `POST /rag/chat`. The unused buggy `app/core/rate_limit_redis.py` was removed. Verified by `tests/test_quota.py` (13 tests using fakeredis: tier mapping, demo-mode no-op, no-Redis no-op, under/at/over quota, 429 payload, enterprise bypass, fail-closed on broken Redis, TTL set on first increment) and `tests/test_quota_wired_into_routers.py` (4 tests inspecting FastAPI's dependency tree on each write route — these will fail any future refactor that disconnects the quota).
