...
- **Blocks 4–8 (2026-05-23):** detailed per-block narratives live in `KIRO_SESSION_BOOTSTRAP.md` Section 3 ("Block sequence completed"), per the source-of-truth shift documented in this file's header. Headline: Block 4 = pool sizing + OTel + Celery structured logs + SLOs (PR #16); Block 5 = TOTP + GDPR soft-delete + MCP hardening (PR #17); Block 6 = nexmem-admin CLI + force-logout + impersonation + analytics (PR #18); Block 7 = app metrics + suspension + queue backpressure + retention (PR #19, migrations 023+024); Block 8 = nexmem-py / nexmem-js publish readiness + frontend auth audit (PR #20, no migrations). Sandbox test count grew from 202 to 258 across this stretch. Two new operator actions (#11 PyPI publish, #12 npm publish) were added to Section 9 in Block 8.
- **Block 9 (2026-05-24):** SDK developer-onboarding docs on `docs/sdk-quickstarts` stacked on Block 8 tip `ffc80d7`. Single code commit `51bf9c9` adds runnable local end-to-end quickstarts for both SDKs (`examples/python_quickstart.py`, `examples/javascript_quickstart.mjs`), an `examples/README.md` index, "Local development" sections in both SDK READMEs, a root-README discoverability link, and six docs-shape regression tests in `tests/test_quickstart_examples.py` (file-existence, `nxm_` prefix only / no legacy `mem_` or `sk_live_`, `localhost:8000` target, no copy-pasted real-looking key literal, README cross-links). Pure docs block — no backend, SDK-source, or package-metadata changes; no migrations; **no new operator actions** (publishes #11 and #12 stay as-is). Sandbox: backend pytest **264 passed** / 0 failed / 33 skipped / 5 deselected (+6 quickstart tests); nexmem-py **8 passed**; nexmem-js **6 passed**; total **278 / 0 fail**. `python scripts/scan_secrets.py`: clean. The R-102 doc/work-log drift surfaced during session bootstrap was deliberately left out of scope per the user's Block 9 spec guard.
