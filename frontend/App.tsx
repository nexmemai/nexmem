...
import {
  ...
  Check, X, Github, Twitter, MessageCircle, ArrowRight, Menu, Bot,
  Coins, Terminal, Headphones, Star, Lock, Database, Cpu, Activity,
  ChevronRight, Shield, Boxes, Download, ChevronDown, PlayCircle
} from "lucide-react";
...
/* ─────────────────── SHARED PRIMITIVES ─────────────────── */
function GlowButton({ children, className = "", onClick }) {
  return (
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-[#080812]/80 backdrop-blur-xl border-b border-white/5" : "bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm text-gray-300">
            ...
            <a href="#" className="hover:text-white transition flex items-center gap-1.5"><Github size={14}/>GitHub</a>
          </div>
...
/* ─────────────────── STICKY BAR ─────────────────── */
function StickyBar({ heroVisible }) {
...
/* ─────────────────── HERO ─────────────────── */
function HeroSection({ mousePos, setMousePos }) {
  ...
  const fade = (d) => ({ transition: `all .7s ease ${d}ms`, opacity: loaded ? 1 : 0, transform: loaded ? "translateY(0)" : "translateY(28px)" });
  return (
...
/* ─────────────────── CONSTELLATION ─────────────────── */
function Constellation({ mousePos }) {
  const memTypes = [
...
function HowItWorks() {
  const steps = [
    { n:"01", title:"Install the SDK", code:"npm install @nexmem/sdk", color:"#6C63FF", icon:Download },
...
function CodeTerminal({ title, subtitle, code, accent, ry }) {
  ...
  const cell = (val, tipKey) => {
    ...
    return (
      <div className="tip-wrap mx-auto w-fit">
        ...
        <div className="tip-box">{partialTips[tipKey]}</div>
      </div>
    );
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-5 gap-10 mb-12">
          <div className="md:col-span-2">
            ...
            {/* CHANGE 10d */}
            <p className="text-sm text-gray-500 max-w-xs">Memory infrastructure for the agentic web.</p>
          </div>
          {[
